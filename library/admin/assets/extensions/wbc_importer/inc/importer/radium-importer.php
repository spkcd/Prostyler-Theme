	...
	class Radium_Theme_Importer {
		...
		public function process_imports() {
			...
			if (!empty($this->content_demo) && is_file($this->content_demo)) {
				cbt_apis::poll('Importing Wordpress data', 'important');
				$this->set_demo_data($this->content_demo);
			}

			if (!empty($this->theme_options_file) && is_file($this->theme_options_file)) {
				cbt_apis::poll('Importing theme settings', 'important');
				$this->set_demo_theme_options($this->theme_options_file);
			}

			//cbt_apis::poll('Importing menus', 'important');
			$this->set_demo_menus();

			if (!empty($this->widgets) && is_file($this->widgets)) {
				cbt_apis::poll('Importing widgets', 'important');
				$this->process_widget_import_file($this->widgets);
			}

			// yunus edit - added after process import action
			do_action('cbt_after_all_import_processes', $this->active_import, $this->demo_files_path);
		}
		...
		public function set_demo_theme_options($file) {
			...
			if (!empty($data) || is_array($data)) {
				// ------------------------------------------------------
				//  yunus edit - backup theme options
				// ------------------------------------------------------
				global $cbt_importer;
				$cbt_importer->backup_theme_options();

				// ------------------------------------------------------
				//  yunus edit - merge old data with new so defaults are not added back
				// ------------------------------------------------------
				$old_data = get_option($this->theme_option_name);
				// echo '<pre>before: '.print_r($old_data, TRUE).'</pre>';
				$data = array_replace_recursive( $old_data, $data );
				unset( $data['redux-backup'] );

				// echo '<pre>after: '.print_r($data, TRUE).'</pre>';die();
			...
			foreach ($data as $sidebar_id => $widgets) {
				...
				foreach ($widgets as $widget_instance_id => $widget) {
					...
					if (!$fail) {
						...
						global $cbt_importer;
						$cbt_importer->log_widget($use_sidebar_id, $new_instance_id);
